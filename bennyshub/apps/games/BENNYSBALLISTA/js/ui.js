...
RT.ui = (function () {
  ...
  let kingdom=null,kingdomSource='levels',exploreIndex=0,exploreReturn=null,editorCastleId=null;
  ...
  function menu(){
    ...
    if(screen==='settings'){
      ...
      title='Castle Workshop · mouse and keyboard needed';hint='Mouse and keyboard required. Switch scanning will not work inside the editor. Cancel stays in the game.';items=[{label:'Cancel',action:()=>{screen=editorCastleId?'kingdom':'settings';menu();}},{label:'Open Castle Workshop',sub:'Mouse and keyboard needed',action:()=>{window.location.href='editor.html'+(editorCastleId?'?castle='+encodeURIComponent(editorCastleId):'');}}];
      ...
      if(kingdom.level)items.splice(items.length-1,0,{label:'Export JSON',action:()=>{try{RT.courses.download([kingdom.level]);speak('Castle JSON downloaded. Keep it locally or upload it to your own storage.');}catch(error){speak(error.message);}}},{label:'Edit in Workshop',action:()=>{editorCastleId=kingdom.id;screen='editorWarning';menu();}});
