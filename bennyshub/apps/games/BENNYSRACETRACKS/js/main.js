...
(function () {
  ...
  function init() {
    ...
    camera = new THREE.PerspectiveCamera(62, window.innerWidth / window.innerHeight, 0.5, 1400);
  ...
  function loop(now) {
    ...
    try {
      ...
      console.error('Race Tracks frame error:', err);
      running = false;
      const el = U.$('loading');
